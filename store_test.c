...
static int test_empty_section_helper(
 ...
 // unsigned long expected_ret,
  struct aisle_section expected_ret,
  int zero_out_aisles
) {
//  unsigned long result;
  struct aisle_section result_info;
  ...
  result_info = empty_section_with_id(id);
  ...
  if ((result_info.aisle_index != expected_ret.aisle_index) || (result_info.section_index != expected_ret.section_index)) {
    printf("\t\terror: wrong return when id=0x%04hx -- actual return: aisle = %d, section = %d,  expected return: aisle = %d, section = %d\n", id, result_info.aisle_index, result_info.section_index, expected_ret.aisle_index, expected_ret.section_index);
    if (zero_out_aisles) {
...
static int empty_section_with_id_tests() {
  ...
  if (!grade_output) printf("\tTesting specific cases...\n");
  struct aisle_section expected_result;
  ...
  expected_result.aisle_index = -1;
  expected_result.section_index = -1;
...
//    (unsigned long)NULL,
  ...
  expected_result.aisle_index = 4;
  expected_result.section_index = 2;
  errors += test_empty_section_helper(
    2,
    4,
    0x080008000C000FFF,
//    (unsigned long)(((unsigned short*)&aisles[4]) + 2),
  ...
  expected_result.aisle_index = 4;
  expected_result.section_index = 3;
  errors += test_empty_section_helper(
    2,
    4,
    0x080008010C000FFF,
//    (unsigned long)(((unsigned short*)&aisles[4]) + 3),
  ...
  for (int i = 0; i < 500; i++) {
    ...
    if (index < 10) {
      ...
      aisles[index] = set_spaces_sol(aisles[index], section_index, 0);
      aisles_before[index] = aisles[index];
    }

 //   unsigned long solution = (unsigned long) empty_section_with_id_sol(id);
    expected_result = empty_section_with_id_sol(id);

    if (test_empty_section_helper(id, 0, aisles[0], expected_result, 0) != 0) {
      errors += -1;
...
//  result = (unsigned long) section_with_most_items();
  result_info = section_with_most_items();
  ...
  if ((result_info.aisle_index != expected_ret.aisle_index) || (result_info.section_index != expected_ret.section_index)) {
    printf("\t\terror: wrong return -- actual return: aisle = %d, section = %d,  expected return: aisle = %d, section = %d\n", result_info.aisle_index, result_info.section_index, expected_ret.aisle_index, expected_ret.section_index);
...
//    (unsigned long)(((unsigned short*)&aisles[4]) + 1),
  ...
  expected_result.aisle_index = 0;
  expected_result.section_index = 0;
  errors += test_most_items_helper(
    1,
    0x0000000000000000,
...
//    unsigned long solution = (unsigned long) section_with_most_items_sol();
    expected_result = section_with_most_items_sol();

    if (test_most_items_helper(0, aisles[0], expected_result, 0) != 0) {
      errors += -1;
      if (!grade_output) print_before_picture(0);
      break;
    }
...
// Only two command line flags (-f test_name -g produce gradescope output)
